...
#include "details/hash_map_iterator.hpp"
...
namespace details
{
    ...
    [[nodiscard]] constexpr auto bucket_iterator_to_iterator(
        const cppshiz::details::bucket_iterator<Key, T, Container, isConst, projectToConstKey>& bucket_it,
        Nodes& nodes) -> cppshiz::details::dense_hash_map_iterator<Key, T, Container, isConst, projectToConstKey>
    {
        if (bucket_it.current_node_index() == cppshiz::details::node_end_index<Key, T>)
        {
            return cppshiz::details::dense_hash_map_iterator<Key, T, Container, isConst, projectToConstKey>{
                nodes.end()};
        }
        else
        {
    ...
    inline constexpr bool is_transparent_key_equal_v =
        cppshiz::details::is_detected<detect_transparent_key_equal, Hash>::value;

    template <class Pred>
    using detect_is_transparent = typename Pred::is_transparent;

    template <class Pred>
    inline constexpr bool is_transparent_v = cppshiz::details::is_detected<detect_is_transparent, Pred>::value;
    ...
    template <class Alloc>
    inline constexpr bool is_iterator_v = cppshiz::details::is_detected<detect_allocate, Alloc>::value&&
        cppshiz::details::is_detected<detect_value_type, Alloc>::value;
...
template <
    ...
    class GrowthPolicy = cppshiz::details::power_of_two_growth_policy>
class dense_hash_map : private GrowthPolicy
{
private:
    using node_type = cppshiz::details::node<Key, T>;
    ...
    using node_index_type = cppshiz::details::node_index_t<Key, T>;
    ...
    static inline constexpr node_index_type node_end_index = cppshiz::details::node_end_index<Key, T>;
...
public:
    ...
    using iterator = cppshiz::details::dense_hash_map_iterator<Key, T, nodes_container_type, false, true>;
    using const_iterator =
        cppshiz::details::dense_hash_map_iterator<Key, T, nodes_container_type, true, true>;
    using local_iterator = cppshiz::details::bucket_iterator<Key, T, nodes_container_type, false, true>;
    using const_local_iterator = cppshiz::details::bucket_iterator<Key, T, nodes_container_type, true, true>;
...
template <
    ...
    class = details::require_not_allocator<Pred>, class = details::require_allocator<Alloc>>
dense_hash_map(
    std::initializer_list<std::pair<Key, T>>, cppshiz::details::node_index_t<Key, T> = 8, Hash = Hash(),
...
dense_hash_map(
    InputIt, InputIt,
    cppshiz::details::node_index_t<details::iter_key_t<InputIt>, details::iter_val_t<InputIt>>, Alloc)
    ->dense_hash_map<
...
dense_hash_map(
    InputIt, InputIt,
    cppshiz::details::node_index_t<details::iter_key_t<InputIt>, details::iter_val_t<InputIt>>, Hash, Alloc)
    ->dense_hash_map<
...
dense_hash_map(std::initializer_list<std::pair<Key, T>>, cppshiz::details::node_index_t<Key, T>, Alloc)
...
template <
    ...
    class = details::require_not_allocator_and_integral<Hash>,
    class = details::require_allocator<Alloc>>
dense_hash_map(std::initializer_list<std::pair<Key, T>>, cppshiz::details::node_index_t<Key, T>, Hash, Alloc)
...
constexpr void swap(
    cppshiz::dense_hash_map<Key, T, Hash, Pred, Allocator, GrowthPolicy>& lhs,
    cppshiz::dense_hash_map<Key, T, Hash, Pred, Allocator, GrowthPolicy>&
...
constexpr void erase_if(cppshiz::dense_hash_map<Key, T, Hash, KeyEqual, Alloc>& c, Pred pred)
{
