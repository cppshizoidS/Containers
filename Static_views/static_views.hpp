#ifndef TOOLS_STATIC_VIEWS_HPP
#define TOOLS_STATIC_VIEWS_HPP

#include "stack_flat_map.hpp"
...
template <typename Value> struct oversized_array {
  ...
  constexpr auto end() const noexcept {
    return std::next(data.begin(), static_cast<std::ptrdiff_t>(size));
  }
};
...
consteval auto to_string_view(creates_string_like auto callable) {
  ...
  std::basic_string_view<Value_Type> result(static_data.begin(),
                                            static_data.end());
  return result;
}
...
template <std::size_t MaxSize> constexpr auto stackify(auto value) {
  return value;
}

template <std::size_t MaxSize, typename CharType, std::size_t CurSize>
constexpr auto stackify(const basic_stack_string<CharType, CurSize> &string) {
  return string;
}

template <std::size_t MaxSize, typename CharType>
constexpr auto stackify(const std::basic_string<CharType> &string) {
  return basic_stack_string<CharType, MaxSize>{string.begin(), string.end()};
}

template <std::size_t MaxSize, typename Value>
constexpr auto stackify(const std::vector<Value> &vec) {
  return simple_stack_vector<decltype(stackify<MaxSize>(std::declval<Value>())),
                             MaxSize>{vec.begin(), vec.end()};
}
...
constexpr auto stackify(const flat_map_adapter<Key, Value, Container> &map) {
  return stack_flat_map<decltype(stackify<MaxSize>(std::declval<Key>())),
                        decltype(stackify<MaxSize>(std::declval<Value>())),
                        MaxSize>{map.begin(), map.end()};
}

template <std::size_t MaxSize, typename Key, typename Value,
          std::size_t CurSize>
constexpr auto stackify(const stack_flat_map<Key, Value, CurSize> &map) {
  return stack_flat_map<decltype(stackify<MaxSize>(std::declval<Key>())),
                        decltype(stackify<MaxSize>(std::declval<Value>())),
                        CurSize>{map.begin(), map.end()};
}

template <typename Arg> constexpr auto max_max(const Arg &lhs, const Arg &rhs) {
  return std::max(lhs, rhs);
}

template <typename First, typename Second>
constexpr auto max_max(const pair<First, Second> &lhs,
...
constexpr auto
max_element_size(const basic_stack_string<CharType, CurSize> &string) {
  return string.size();
}

template <typename Value, std::size_t CurSize>
constexpr auto
max_element_size(const simple_stack_vector<Value, CurSize> &vec) {
  decltype(max_element_size(std::declval<Value>())) child_max{};

  for (const auto &elem : vec) {
    child_max = max_max(child_max, max_element_size(elem));
  }
...
constexpr auto
max_element_size(const stack_flat_map<Key, Value, CurSize> &map) {
...
template <auto NewSize> constexpr auto resize(const auto &value) {
  return value;
}

template <auto NewSize, typename CharType, std::size_t CurSize>
constexpr auto resize(const basic_stack_string<CharType, CurSize> &str) {
  // +1 for null terminator
  return basic_stack_string<CharType, NewSize + 1>{str.begin(), str.end()};
}
...
constexpr auto resize(const simple_stack_vector<Value, CurSize> &vec) {
  using new_value_type =
      decltype(resize<NewSize.second>(std::declval<Value>()));
  return simple_stack_vector<new_value_type, NewSize.first>{vec.begin(),
                                                            vec.end()};
}

template <auto NewSize, typename Key, typename Value, std::size_t CurSize>
constexpr auto resize(const stack_flat_map<Key, Value, CurSize> &map) {
  using new_key_type =
      decltype(resize<NewSize.second.first>(std::declval<Key>()));
  using new_value_type =
      decltype(resize<NewSize.second.second>(std::declval<Value>()));

  return stack_flat_map<new_key_type, new_value_type, NewSize.first>{
      map.begin(), map.end()};
}
